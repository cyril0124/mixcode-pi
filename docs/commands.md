...
| `/hide-thinking` | - | `[global]` | Toggles whether thinking blocks are hidden. Hidden blocks render as `Thinking...` (or a `setHiddenThinkingLabel` override); set `ui.boxedHiddenThinking` in [mixcode_settings.json](mixcode-settings.md) for the boxed 3-row tail. |
