...
| `/group-colored-tabs` | - | Workdir | Reorders tabs so colored tabs lead the tab bar, aggregated by color in palette order, with uncolored tabs keeping their relative order after them. Persisted to `mixcode_state.json` and published to other instances in the workdir. See [Tab colors](workspace-and-tabs.md#tab-colors). |
| `/follow-up` | `[--batch] [text]` | Session | Adds a separate follow-up round; `--batch` groups it with adjacent batch rounds; without text, resumes queued follow-ups. See [queue semantics and resume errors](queue-and-follow-up.md). |
| `/compact` | `[provider/modelId] [instructions]` | Session | Manually triggers context compaction on the current branch. An optional first argument in `provider/modelId` form runs compaction with that model, then restores the original model. The remaining text is passed as custom compaction instructions. |
...
| `/hide-thinking` | - | `[global]` | Toggles between full thinking and the configured preview. Preview defaults and the `Thinking...` placeholder option are defined in [settings](mixcode-settings.md). |
...
| `/context` | - | Session | Opens a read-only context usage panel: a grid plus a legend that splits the current context into System prompt, Project context, Skills, Tool guidelines, Tool schemas, and Messages, with free space and the autocompact buffer. The total is the last response's reported usage plus an estimate for the messages after it; before any response reports usage, the panel marks the total with `~` and the percentage with `?`. The window is the tab's `/context-limit` value. Every category the session has is listed, including those holding no tokens; Messages appears once a total is known. Before the first request the split comes from the prompt MixCode assembles, so the prompt categories are attributed from the start; extension-injected sections appear once a request has collected them. The grid drops to a compact width on narrow terminals and is omitted when only the numbers fit. |
| `/console-history` | - | - | Opens the latest 1,000 bridged `console.log/info/debug/warn/error` records from the current `mpi` process, each prefixed with its `YYYY-MM-DD HH:MM:SS` timestamp and, when the line came from a tab's work (agent turn, extension command, shell execution), the emitting tab title in brackets. Startup and background-timer output has no tab title. Checks the project setting, global setting, `$VISUAL`, and `$EDITOR`, in that order. The selected command must pass a one-second `--version` probe. If none is set, MixCode tries `nvim` and then `vim`. If no external editor is available, MixCode opens the built-in read-only viewer. Restarting `mpi` clears the history. |
