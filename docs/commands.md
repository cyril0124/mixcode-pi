...
| `/follow-up` | `<text>` | Session | Adds a batch follow-up. See [queue semantics](queue-and-follow-up.md#follow-up-rounds). |
| `/follow-up-next` | `[text]` | Session | Adds a separate follow-up round; without text, resumes queued follow-ups. See [queue semantics and resume errors](queue-and-follow-up.md). |
