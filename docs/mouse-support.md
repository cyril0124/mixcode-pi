...
Agent tool call rows and summary cards (compaction and branch) in the chat transcript respond to the pointer. Hovering one paints its block with the selection background (`theme.selectedBg`), and clicking it expands or collapses that block alone, leaving the other blocks as they were. `ctrl+o` keeps its own meaning and expands every block at once. The renderer publishes the rows, height, and expansion target of every visible block (`chatPointerBlockRanges`); the block under the pointer is recorded by its first row in `chatHoverRow` and the cue comes from `paintRowBackground`; a primary press records the cell it landed on (`chatClickCell`), and a release on that same cell turns the press into a click, so dragging to select text never expands a block, while the cue stays on the block the pointer is over. Selected cells inside a pointer block add an underline to that background, so a selection stays readable there. `paintRowBackground` never underlines anything. Text outside such a block keeps the plain selection background. A click on a row that holds no pointer block does nothing, and an open overlay owns the click instead. A `!` command the user typed renders through its own path and answers no pointer. Tool blocks and summary cards keep separate expansion state (`expandedToolCalls`, `expandedSummaryCards`); a summary card is keyed by its session entry's role and timestamp, so it stays expanded after a reload. The tab bar, metadata badges, and the scrollbar keep their own hover and click behaviour.
...
- **Block Background And Selection Underline**: `chatSelectionHighlight` paints a selected cell of a pointer block with `theme.selectedBg` and an underline, and `applyChatSelectionHighlight` reapplies the background after every SGR in the slice, so tool and thinking card backgrounds cannot cover the selection. The underline keeps it readable under the cue.
