...
Tool call rows in the chat transcript respond to the pointer. Hovering one paints its block with the hover background, and clicking it expands or collapses that tool call alone, leaving the other calls as they were. `ctrl+o` keeps its own meaning and expands every tool block at once. The renderer publishes the rows and tool call id of every visible block (`chatToolRowRanges`); the block under the pointer is recorded by its first row in `chatHoverRow` and the cue comes from `paintRowBackground`; a primary press records the cell it landed on (`chatClickCell`), and a release on that same cell turns the press into a click, so dragging to select text neither highlights nor expands anything. A block whose rows overlap a live text selection keeps its selection colours instead of the hover cue, because `paintRowBackground` and the selection paint the same background: without this, selecting text in a block would hide the columns it selected. A click on a row that holds no tool call does nothing, and an open overlay owns the click instead. The tab bar, metadata badges, and the scrollbar keep their own hover and click behaviour.
