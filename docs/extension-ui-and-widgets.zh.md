...
MixCode Pi 为 Pi 扩展提供完整的 UI 宿主原语支持（`src/agent/runtime-extension-ui.ts`），包括多区域组件停靠区（`src/agent/runtime-extension-widgets.ts`）、折叠式扩展侧边栏以及内联组件模式。
...
## 2. 内联组件模式 (`/toggle-inline-widgets`)
