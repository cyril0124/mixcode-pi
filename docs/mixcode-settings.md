...
| `ui.boxedHiddenThinking` | boolean | `false` | With Pi `hideThinkingBlock` on: render each hidden thinking block as a boxed 3-row tail that follows the stream instead of the `Thinking...` placeholder. The rounded frame uses `borderMuted`; the title and non-italic body use `thinkingText`, with one column of padding on each side of the body. The title carries a live timer while the block streams and freezes it once thinking stops (a tool call or answer text follows, or the message ends). Durations below 1s use integer milliseconds (`320ms`); from 1s to under 60s, live values truncate to tenths (`1.4s`) and frozen values round to hundredths (`1.42s`). At 60s and above, durations use whole-second minute/hour labels (`1m 05s`). A `setHiddenThinkingLabel` override still replaces the whole block. Editable via `/settings` as "Boxed hidden thinking". |
