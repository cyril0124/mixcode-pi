...
| `/hide-thinking` | - | `[global]` | 切换是否隐藏 Thinking 块。隐藏时显示 `Thinking...`（或 `setHiddenThinkingLabel` 覆盖）；要显示带框 3 行尾窗，需在 [mixcode_settings.json](mixcode-settings.zh.md) 中设置 `ui.boxedHiddenThinking`。 |
