...
| `mpi-transcript` | `/transcript [context\|chatlog\|thinking\|latest-agent\|latest-user] [N] [full]` | Views transcript slices (effective LLM context, chatlog, thinking, latest messages; optional `N` = last N turns; `full` = untruncated tool output on chatlog/context, any position) in `$VISUAL` / `$EDITOR` or the in-app viewer. The `context` view opens with a chars/4 size estimate — `system prompt + tool schemas + all messages`, plus the share of the context window when the model's window is known; it always covers the whole context, so `N` cuts the display but not the number. nvim adds a User/Assistant winbar (`[t`/`]t` jump turns), per-role heading colors, a current-turn gutter bar, folded tool in/out fences, statuscolumn role marks, dimmed meta/thinking, wrap/linebreak, and `conceallevel=2`. |
