...
| `mpi-stuck-guard` | Auto on high-cardinality search; `/stuck-guard`, `/stuck-guard config`, `/stuck-guard stats`, `<agentDir>/mpi-stuck-guard.json` | Guard against stuck sessions: blocks recursive searches of root/home directories, and watches provider streams for first-event and idle timeouts with retry cooldowns, Provider filtering, and current-session statistics. |
