...
| `mpi-transcript` | `/transcript [chatlog\|context\|thinking\|latest-agent\|latest-user] [N]` | Views transcript slices (full chatlog, post-compaction LLM context, thinking, latest messages; optional trailing `N` = last N turns) in `$VISUAL` / `$EDITOR` or the in-app viewer. |
