...
| `mpi-transcript` | `/transcript [context\|chatlog\|thinking\|latest-agent\|latest-user] [N]` | Views transcript slices (effective LLM context, full chatlog, thinking, latest messages; optional trailing `N` = last N turns) in `$VISUAL` / `$EDITOR` or the in-app viewer. |
