...
| `mpi-stuck-guard` | `/stuck-guard`, `/stuck-guard config`, `/stuck-guard stats`, `<agentDir>/mpi-stuck-guard.json` | Provider stream watchdog for first-event and idle timeouts, retry cooldowns, Provider filtering, and current-session statistics. |
