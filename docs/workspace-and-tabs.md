...
On a colored tab the chip background comes from the color, and the chip foreground is the color's contrast pair. The theme's active, recency, and completion backgrounds do not apply. The active chip shimmers, and its wave inverts the lit cells with reverse video, because the theme's text and accent colors have no contrast over a chip color. The title keeps its bold weight in every phase of the sweep. A done or unread chip keeps the contrast pair too, because the theme's success color stays under 3:1 against all eight chip colors; completion reads from the `✓` glyph and the bold title. Other status colors (running, waiting, error) stay shape-only. The active tab still carries its focus marker. Uncolored chips use `▌` in the theme's border color. A colored chip uses `▐` in the color's contrast pair, which fills the inner half of the cell so the chip keeps its own color on the outer edge; the mark can then never be mistaken for the surrounding background. Uncolored tabs keep the theme's chip styling. Home's agent card paints the same color behind its title segment.
