...
| `ui.boxedHiddenThinking` | 布尔值 | `false` | 在 Pi `hideThinkingBlock` 打开时：每个隐藏的 thinking 块渲染为跟随流式输出的带框 3 行尾窗，替代 `Thinking...` 占位符。圆角边框使用 `borderMuted`，标题和非斜体正文使用 `thinkingText`，正文左右各留一格。标题在流式期间实时计时，思考一结束（后续 tool call / 文本出现或消息结束）即冻结。不足 1 秒显示整数毫秒（`320ms`）；1 秒至不足 60 秒，实时值截断到一位小数（`1.4s`），冻结值四舍五入到两位小数（`1.42s`）；达到 60 秒后使用整秒的分/时格式（`1m 05s`）。`setHiddenThinkingLabel` 覆盖仍整块替换。在 `/settings` 中作为 “Boxed hidden thinking” 可编辑。 |
