...
| `ui.boxedHiddenThinking` | 布尔值 | `false` | 在 Pi `hideThinkingBlock` 打开时：每个隐藏的 thinking 块渲染为跟随流式输出的带框 3 行尾窗，替代 `Thinking...` 占位符；`setHiddenThinkingLabel` 覆盖仍整块替换。在 `/settings` 中作为 “Boxed hidden thinking” 可编辑。 |
