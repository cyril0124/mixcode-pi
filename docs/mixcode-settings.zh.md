...
| `ui.showResponseModelNotices` | 布尔值 | `true` | 响应报告的模型名与请求名不同时显示提示。可在 `/settings` 的 MixCode 分区通过 “Response model notices” 修改，立即影响所有已打开的标签页，包括已恢复的消息。隐藏提示仍保留消息元数据。详见[采集与显示规则](model-management.zh.md#响应报告的模型名)。 |
...
| `editorPaddingX` | 非负整数 | 未设置 → `1` | 输入编辑器侧边框内的正文水平内边距。 |
...
- `ui.showResponseModelNotices`：必须为布尔值；启动与原始设置读取入口均拒绝非法值。
