...
已上色 Tab 的 chip 背景来自该颜色，前景是配对的对比色。主题的选中/最近访问/完成背景不再生效。当前 chip 仍有扫光，扫光对命中格子做反色（reverse video），因为主题的文字色与强调色在 chip 底色上没有对比保证；标题在整个扫光周期内保持加粗。已完成或未读的 chip 同样保留该对比色，因为主题成功色在八种 chip 底色上的对比度都低于 3:1，完成状态由 `✓` 字形与加粗标题表达；其他状态色（运行中/等待输入/报错）只保留字形。当前 Tab 仍以焦点标记区分。未上色的 chip 用主题边框色的 `▌`；上色 chip 用对比色的 `▐`，该字形只填充格子内侧一半，chip 自己的颜色留在外侧边缘，因此竖条不会被与背景同色的对比色吞掉。未上色的 Tab 仍使用主题样式。Home 的 Agent 卡片在标题段背景上同步该颜色。
