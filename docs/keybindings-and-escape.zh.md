...
| `Ctrl+J` / `Ctrl+K` | Tab 跳转 | 下一个 / 上一个结果 | 向下 / 向上移动 Tab 跳转选择项。 |
...
| `Alt+Enter` | 输入框 | Follow-up | Agent 忙碌或后续队列暂停时入队，否则正常提交。详见[队列管理](queue-and-follow-up.zh.md)。 |
...
用户按下 Escape
        ...
        │                          轮次一律普通中断）
        └─ 6. 输入框为空且空闲？ ──> 500ms 内连按两次 Esc 打开会话树或 fork。
                                       Vim 模式下无效。
```
