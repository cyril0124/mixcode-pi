...
| 主要快捷键和交互 | `docs/architecture.md` 列出全局和局部作用域快捷键；`src/core/keymap.ts` 提供 scoped keymap；`test/runtime-ui.test.ts` 断言 `file-picker/picker/command-palette/tab-jump/export/question/preview/shell` 作用域存在；`test/ui-commands.test.ts` 覆盖 tab、palette、export、question、queue Esc、Ctrl+Q、@ picker、shell、preview；shell 焦点下 `Ctrl+V/L/E` 透传给 shell、`Ctrl+P` 保留 command palette。 | 已验证 |
...
| 真实 TUI 截图/交互验证 | `tmp/ref-mixcode-160x48.*`、`tmp/mixcode-pi-*.txt/.ansi`、`tmp/tui-verify-*` 记录参考和当前 TUI 截图；`test/tui-smoke.test.ts` 可重复验证 180x48 tmux 启动、标题、OpenCode 反残留、`/theme li`、command palette、tab jump、`@` file picker、preview、shell、新建 tab、mouse tab click 和 Ctrl+Q 退出。 | 已验证 |
| 视觉风格接近 MixCode | `src/ui/themes.ts` 提供 MixCode dark/light/terminal；render tests 断言 light/dark palette；tmux 截图文件保存 160x48 对照。 | 强验证但仍需人工审美复核 |
...
| Theme 切换 | `/theme` picker、`/theme light`、`/theme li`、`/theme terminal`、extension `ctx.ui.setTheme("light")` 均有测试；tmux 180x48 逐字输入 `/theme li` 后 Enter 确认 light palette。 | 已验证 |
...
tmux TUI smoke:
  viewport: 180x48
  pass: starts, rejects OpenCode UI, switches /theme li to light palette
