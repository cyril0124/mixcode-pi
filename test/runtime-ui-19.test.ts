...
test("runtime marks goals complete when assistant emits completion marker", async () => {
  const dir = await mkdtemp(join(tmpdir(), "mixcode-runtime-goal-"));
  try {
