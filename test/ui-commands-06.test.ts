...
test("global key input toggles MixCode overlays and passes through regular input", async () => {
  ...
  assert.deepEqual(handleMixCodeKeyInput(state, "\x1b[27;5;47~", tui), { consume: true });
  assert.match(overlays.at(-1) ?? "", /MixCode Help/);
  assert.match(overlays.at(-1) ?? "", /Ctrl\+P/);
  assert.match(overlays.at(-1) ?? "", /Ctrl\+L/);
  assert.equal(state.helpOpen, true);
  assert.deepEqual(handleMixCodeKeyInput(state, "q", tui), { consume: true });
  assert.equal(state.helpOpen, true);
  assert.doesNotMatch(overlays.at(-1) ?? "", /Exit requested/);
