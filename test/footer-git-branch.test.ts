...
async function initTempGitRepo(): Promise<string> {
  ...
  await execFileAsync("git", ["config", "user.email", "t@t.com"], { cwd: workdir });
  await execFileAsync("git", ["config", "user.name", "t"], { cwd: workdir });
  await fsPromises.writeFile(path.join(workdir, "f"), "x\n");
  await execFileAsync("git", ["add", "f"], { cwd: workdir });
  await execFileAsync("git", ["commit", "-qm", "init"], { cwd: workdir });
  await execFileAsync("git", ["branch", "-M", "base-branch"], { cwd: workdir });
  return workdir;
}
