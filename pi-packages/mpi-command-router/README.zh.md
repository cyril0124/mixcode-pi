...
`/command-router` 显示两层配置的生效状态。`/command-router off` 与 `/command-router on` 写入所有适用的层，`--global` 或 `--project` 可把切换限定到单层；仓库层没有配置文件时不会被创建，除非显式指定 `--project`。切换只改 `enabled`，保留全部路由与 `$schema`，拒绝覆盖不可读或不合法的文件并返回 `Error:` 消息，同时指出仍在阻止路由的层。
