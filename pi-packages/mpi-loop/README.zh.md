...
`--max-runs N` 在创建时设置总次数，放在 Prompt 之前；时间间隔可放在其前或其后，因此 `30m --max-runs 3 x` 与 `--max-runs 3 30m x` 等价。省略该参数时不限次数。`N` 必须是十进制正安全整数，最大为 `9007199254740991`。缺少参数值、非法次数或 Prompt 之前重复指定参数，均报 `Error:`，不创建任务。
...
未使用 `--` 时，省略开头时间间隔的形式支持末尾时间子句，例如 `/loop --max-runs 3 check every 2 hours`。紧随 `--max-runs N` 之后的时间间隔 token，或明确指定的开头时间间隔，优先于末尾子句。

## 管理页布局

底部挂件恢复为原来的表格布局，包含 `ON`、`ID`、`M`、`NAME`、`INTERVAL`、`PROMPT`、`NEXT` 和 `RUNS` 列。表头、运行勾选标记和模式标记让每个字段的含义更清楚。`NEXT` 显示更精确的倒计时，例如 `in 1h59m`；有限次数显示为 `1/3`。

管理列表每个任务占两行。第一行显示 ID、Prompt 摘要和靠右的下次执行时间；第二行显示间隔、执行次数及冲突模式。窄窗口先省略模式，再省略间隔；倒计时和次数优先于摘要。选中任务的两行使用相同选中背景。`waiting` 使用警告色，表示任务正在等待 Agent 空闲。

详情页将下次执行时间和总次数放在完整、可滚动的 Prompt 上方。高度不足时，先省略次要元数据，再缩减 Prompt 区域。面板高度预算包含边框和提示，滚动时保持完整任务条目与底部提示可见。宽度不足时隐藏次要快捷键提示，隐藏提示对应的按键仍然有效。

删除与清空确认替换底部提示，同时保留当前任务内容。只有 `y` 确认，其他任何按键均取消确认，且不会同时执行该按键原来的操作。
...
| `Escape` / `q` | 关闭管理浮层 |
| `Ctrl+U`（列表） | 清空搜索文本 |
| `Down` / `Up`、`j` / `k`（详情页） | Prompt 滚动一行 |
| `Ctrl+D` / `Ctrl+U`（详情页） | Prompt 滚动半页 |
| `PageDown` / `PageUp`（详情页） | Prompt 滚动一页 |
| `End` / `Home`、`G` / `g`（详情页） | 跳到末尾 / 开头 |
| `q`（详情页） | 关闭管理页 |
| `y`（确认中） | 确认删除或清空；其他任何按键均取消 |
