...
test("getGoal is null outside ALS even when session map still holds the goal", () => {
  ...
  assert.equal(
    getGoal(),
    null,
    "without session scope, getGoal must not see other session's memory",
  );
});
...
test("overlay-style getSnapshot must re-enter session key or it shows No active goal", async () => {
  ...
  assert.equal(
    leakedGetSnapshot?.().goal,
    null,
    "reproduces empty overlay while session still has goal",
  );
