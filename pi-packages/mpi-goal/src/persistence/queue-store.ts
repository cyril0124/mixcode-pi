...
export function cloneQueuedGoal(goal: QueuedGoal): QueuedGoal {
  return {
    ...
    postCompletionActions: goal.postCompletionActions
      ? goal.postCompletionActions.map((action) => ({ ...action }))
      : undefined,
  };
}
...
export function restoreQueueHeadForRepair(
  pi: ExtensionAPI,
  goal: QueuedGoal,
  reason: string,
): QueueRepairResult {
  ...
  if (head)
    return { status: "blocked_different_head", queueId: goal.queueId, currentHeadId: head.queueId };
...
export function consumeQueueIdForRepair(
  pi: ExtensionAPI,
  queueId: string,
  reason: string,
): QueueRepairResult {
...
export function replayQueueState(ctx: {
  sessionManager: { getBranch(): unknown[] };
}): GoalQueueRuntimeState {
...
export function persistDequeue(
  pi: ExtensionAPI,
  reason: string,
  metadata: DequeuePersistMetadata = {},
): void {
  pi.appendEntry(STATE_ENTRY_TYPE, {
    version: 1,
    kind: "dequeue",
    ...
    reason,
    rationale: metadata.audit?.rationale,
    authority: metadata.audit?.authority,
    at: Date.now(),
  } as GoalQueueEvent);
}
...
function toQueuedGoal(value: unknown): QueuedGoal | null {
  ...
  const minTimeSecondsBeforeWrapUp = parseOptionalPositiveIntegerField(
    raw.minTimeSecondsBeforeWrapUp,
  );
...
function parseOptionalStringRecordField(
  value: unknown,
): ParsedOptionalField<Record<string, string>> {
  if (value === undefined) return { ok: true, value: undefined };
...
function parseQueuedPostCompletion(
  raw: Record<string, unknown>,
): { ok: true; actions: PostCompletionActionSpec[] | undefined } | { ok: false } {
...
function parseOptionalActionSpecs(value: unknown): ParsedOptionalField<PostCompletionActionSpec[]> {
  ...
  for (const item of value) {
    ...
    if (action.type !== "context.reset" || (action.mode !== "clear" && action.mode !== "summarize"))
      return { ok: false };
