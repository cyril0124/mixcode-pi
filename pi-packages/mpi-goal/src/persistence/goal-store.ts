...
export function persistClearGoal(pi: ExtensionAPI, reason: PiGoalEventReason): MutationResult {
  return persistEvent(pi, {
    kind: "clear",
    goalId: getMutableRuntimeState().goal?.goalId,
    goal: null,
    telemetry: null,
    reason,
  });
...
function applyEvent(state: GoalRuntimeState, event: PiGoalStateEvent): GoalRuntimeState {
  ...
  if (event.goalId && state.goal && event.goalId !== state.goal.goalId && event.kind !== "set")
    return state;
...
function isGoalEvent(value: unknown): value is PiGoalStateEvent {
  ...
  return (
    v.version === STATE_EVENT_VERSION && typeof v.kind === "string" && typeof v.reason === "string"
  );
}
...
function requiredGoalFields(
  value: Record<string, unknown>,
): Pick<GoalState, "goalId" | "objective" | "status"> | null {
