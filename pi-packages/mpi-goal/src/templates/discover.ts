...
export function resolveGoalTemplateByName(
  nameOrAlias: string,
  flags: Record<string, string>,
  args = "",
  root = process.cwd(),
  ...
  if (matches.length > 1)
    return {
      ok: false,
      error: `Ambiguous goal template '${nameOrAlias}' matches: ${matches.map((template) => template.name).join(", ")}.`,
    };
  ...
  try {
    ...
    return {
      ok: true,
      template: { name: template.name, path: template.path, objective, flags: { ...flags }, args },
    };
  } catch (error) {
...
function findTemplates(nameOrAlias: string, root: string): GoalTemplate[] {
  return discoverGoalTemplates(root).filter(
    (template) => template.name === nameOrAlias || template.aliases.includes(nameOrAlias),
  );
}
...
function collectTemplates(root: string, templateDir: string, templates: GoalTemplate[]): void {
  collectMarkdown(templateDir, (filePath) => {
    ...
    const relativeName = stripMarkdownExt(
      path.relative(templateDir, filePath).split(path.sep).join("/"),
    );
    templates.push({
      name: relativeName,
      ...
      commandTimeoutMs: parsePositiveInt(
        parsed.frontmatter.command_timeout_ms,
        DEFAULT_COMMAND_TIMEOUT_MS,
      ),
      commandOutputLimit: parsePositiveInt(
        parsed.frontmatter.command_output_limit,
...
function resolveInlineCommands(text: string, template: GoalTemplate, cwd: string): string {
  return text.replace(/!`([^`]+)`/g, (_match, command: string) => {
    if (!template.allowCommands)
      throw new Error(
        `Template ${template.name} uses inline commands but allow_commands is not true.`,
      );
...
function runCommand(command: string, template: GoalTemplate, cwd: string): string {
  try {
    ...
    return output.length > template.commandOutputLimit
      ? `${output.slice(0, template.commandOutputLimit)}\n[output truncated]`
      : output;
  } catch (error) {
