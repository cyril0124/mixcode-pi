...
import { QUEUE_MESSAGE_TYPE, QUEUE_PROMPT_ID } from "../domain/constants.js";
import { getQueue, getQueueRevision, type QueuedGoal } from "../persistence/queue-store.js";
import type { GoalQueueSteeringReason } from "../domain/types.js";
...
export function sendQueueHandoff(
  pi: ExtensionAPI,
  reason: GoalQueueSteeringReason,
  opts: QueueHandoffOptions = {},
): boolean {
  ...
  const sent = sendQueueSteering(pi, reason, {
...
export function queueSteeringStillValid(message: Record<string, unknown>): boolean {
  ...
  return (
    ...
    getQueue()[0]?.queueId === queueId &&
    getQueueRevision() === queueRevision
  );
}
...
function budgetLine(goal: QueuedGoal): string {
  ...
  if (goal.timeBudgetSeconds !== undefined)
    parts.push(`time_budget_seconds=${goal.timeBudgetSeconds}`);
  if (goal.minTokensBeforeWrapUp !== undefined)
    parts.push(`min_tokens_before_wrap_up=${goal.minTokensBeforeWrapUp}`);
  if (goal.minTimeSecondsBeforeWrapUp !== undefined)
    parts.push(`min_time_seconds_before_wrap_up=${goal.minTimeSecondsBeforeWrapUp}`);
...
function budgetGuidance(goal: QueuedGoal): string[] {
  const hasBudgetMetadata =
    ...
    goal.minTokensBeforeWrapUp !== undefined ||
    goal.minTimeSecondsBeforeWrapUp !== undefined;
  if (!hasBudgetMetadata)
    return [
      "Budget guidance: Budgets: none means omit budget/floor params. Do not pass token_budget, time_budget_seconds, min_tokens_before_wrap_up, or min_time_seconds_before_wrap_up when creating the next goal.",
    ];
  return [
    "Budget guidance: carry only the listed budget/floor metadata exactly. Do not invent token_budget, time_budget_seconds, min_tokens_before_wrap_up, or min_time_seconds_before_wrap_up values.",
  ];
}

function preview(objective: string): string {
  return objective.length > OBJECTIVE_PREVIEW_CHARS
    ? `${objective.slice(0, OBJECTIVE_PREVIEW_CHARS - 1)}…`
    : objective;
}
