import type { CompletionFloorEvaluation } from "./floor.js";
import { formatElapsed, formatTokensCompact } from "./format.js";
import type { FloorValuePassId, GoalState, GoalTelemetrySnapshot } from "./types.js";
...
export const FLOOR_VALUE_PASS_CATALOG: readonly FloorWorkCard[] = [
  {
    ...
    concreteFirstAction:
      "Re-read the objective and map each explicit requirement to a concrete artifact, command, file, or proof.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Attack assumptions, enumerate failure modes, and add or tighten a mitigation, test, or proof row.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Review the work from one concrete alternate perspective and reconcile the consequence into the artifact or design.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Inspect one relevant source and write back the finding only if it changes the design, proof, or artifact.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Run or add one targeted probe that would fail if the main invariant is wrong.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Remove duplication, simplify an API/flow, or tighten wording while preserving behavior.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Inspect one cross-surface compatibility point and add a guardrail, invariant, or acceptance criterion.",
  ...
  },
  {
    ...
    concreteFirstAction:
      "Improve README, issue, comments, closeout, proof links, or handoff evidence with one concrete missing detail.",
...
export function selectFloorWorkCard(context: FloorWorkSelectionContext): FloorWorkCard | undefined {
  ...
  return (
    preferByObjective(candidates, context.goal.objective) ??
    candidates.find((card) => card.id === "requirement_gap_audit") ??
    candidates[0]
  );
}

export function buildFloorContinuationGuidance(context: {
  goal: GoalState;
  telemetry: GoalTelemetrySnapshot | null;
  floor: CompletionFloorEvaluation;
  card: FloorWorkCard;
...
export function buildFloorCompletionRefusal(context: {
  goal: GoalState;
  floor: CompletionFloorEvaluation;
  card: FloorWorkCard;
}): string {
...
export function formatFloorBlock(floor: CompletionFloorEvaluation): string {
  ...
  if (floor.minTokensBeforeWrapUp !== undefined) {
    lines.push(
      `- tokens before wrap-up: ${formatTokensCompact(floor.minTokensBeforeWrapUp - (floor.tokensRemainingBeforeWrapUp ?? 0))} / ${formatTokensCompact(floor.minTokensBeforeWrapUp)}${floor.tokenFloorMet ? " (met)" : ""}`,
    );
  }
  if (floor.minTimeSecondsBeforeWrapUp !== undefined) {
    lines.push(
      `- time before wrap-up: ${formatElapsed(floor.minTimeSecondsBeforeWrapUp - (floor.timeSecondsRemainingBeforeWrapUp ?? 0))} / ${formatElapsed(floor.minTimeSecondsBeforeWrapUp)}${floor.timeFloorMet ? " (met)" : ""}`,
    );
  }
  return lines.join("\n");
}

function preferByObjective(
  candidates: FloorWorkCard[],
  objective: string,
): FloorWorkCard | undefined {
  ...
  if (/compat|replay|queue|budget|ui|security|performance|accessibility/.test(lower))
    ordered.push("compatibility_review");
