import {
  BUDGET_HARD_STOP_MULTIPLIER,
  TIME_BUDGET_WARNING_REMAINING_SECONDS,
  TOKEN_BUDGET_WARNING_REMAINING,
} from "./constants.js";
import type {
  BudgetLimitReason,
  BudgetPressure,
  BudgetPressureKind,
  GoalQueueSteeringReason,
  GoalState,
} from "./types.js";
...
function tokenPressure(goal: GoalState): BudgetPressure {
  ...
  if (goal.tokensUsed >= hardStopBudget(goal.tokenBudget))
    return pressure("tokenHardStop", remaining);
...
function timePressure(goal: GoalState): BudgetPressure {
  ...
  if (goal.timeUsedSeconds >= hardStopBudget(goal.timeBudgetSeconds))
    return pressure("timeHardStop", remaining);
