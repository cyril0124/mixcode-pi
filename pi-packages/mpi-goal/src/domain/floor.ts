...
export function evaluateCompletionFloor(goal: GoalState): CompletionFloorEvaluation {
  const tokenFloorMet =
    goal.minTokensBeforeWrapUp === undefined || goal.tokensUsed >= goal.minTokensBeforeWrapUp;
  const timeFloorMet =
    goal.minTimeSecondsBeforeWrapUp === undefined ||
    goal.timeUsedSeconds >= goal.minTimeSecondsBeforeWrapUp;
  return {
    ...
    timeSecondsRemainingBeforeWrapUp: remaining(
      goal.minTimeSecondsBeforeWrapUp,
    ...
    anyFloorConfigured:
      goal.minTokensBeforeWrapUp !== undefined || goal.minTimeSecondsBeforeWrapUp !== undefined,
  };
}
...
export function validateFloorConfig(config: GoalFloorConfig): string | undefined {
  ...
  if (config.minTimeSecondsBeforeWrapUp !== undefined) {
    const error = validatePositiveInteger(
      ...
      config.minTimeSecondsBeforeWrapUp,
    );
    if (error) return error;
    if (
      config.timeBudgetSeconds !== undefined &&
      config.minTimeSecondsBeforeWrapUp > config.timeBudgetSeconds
    ) {
