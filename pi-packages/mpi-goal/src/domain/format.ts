import { budgetLimitReason } from "./budget.js";
import { evaluateCompletionFloor } from "./floor.js";
import {
  GOAL_USAGE,
  GOAL_USAGE_HINT,
  LONG_OBJECTIVE_HINT,
  MAX_OBJECTIVE_CHARS,
  OBJECTIVE_EXCERPT_CHARS,
} from "./constants.js";
...
export function goalUsageSummary(goal: GoalState): string {
  return [
    ...
    formatTimeFloor(goal),
    formatTokenFloor(goal),
  ]
    .filter(Boolean)
    .join("; ");
}
...
function footerUsage(goal: GoalState): string | undefined {
  ...
  if (floor.anyFloorConfigured && !floor.allFloorsMet) {
    if (floor.minTimeSecondsBeforeWrapUp !== undefined)
      return `floor ${formatElapsed(goal.timeUsedSeconds)} / ${formatElapsed(floor.minTimeSecondsBeforeWrapUp)}`;
    if (floor.minTokensBeforeWrapUp !== undefined)
      return `floor ${formatTokensCompact(goal.tokensUsed)} / ${formatTokensCompact(floor.minTokensBeforeWrapUp)}`;
  }
...
export function goalSummaryLines(goal: GoalState): string[] {
  ...
  if (goal.timeBudgetSeconds !== undefined)
    lines.push(`Time budget: ${formatElapsed(goal.timeBudgetSeconds)}`);
  if (goal.minTokensBeforeWrapUp !== undefined)
    lines.push(
      `Token wrap-up floor: ${formatTokensCompact(goal.tokensUsed)} / ${formatTokensCompact(goal.minTokensBeforeWrapUp)}`,
    );
  if (goal.minTimeSecondsBeforeWrapUp !== undefined)
    lines.push(
      `Time wrap-up floor: ${formatElapsed(goal.timeUsedSeconds)} / ${formatElapsed(goal.minTimeSecondsBeforeWrapUp)}`,
    );
