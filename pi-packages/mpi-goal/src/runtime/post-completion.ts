...
import { persistUpdateGoal } from "../persistence/goal-store.js";
import { notifyWarning } from "../surface/ui/notify.js";
import type {
  GoalState,
  PiGoalEventReason,
  PostCompletionActionSpec,
...
export function createPostCompletionActionStates(
  specs: PostCompletionActionSpec[],
  now = Date.now(),
): PostCompletionActionState[] {
...
export async function runPostCompletionActionsSafely(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  goal: GoalState | null,
  ...
  for (const action of getRunnablePostCompletionActions(current)) {
    current = persistActionState(
      pi,
      current,
      { ...action, status: "running", updatedAt: Date.now() },
      reason,
    );
    const running =
    ...
    if (!result.ok)
      notifyWarning(
        ctx,
        `Post-completion action ${running.type} failed but goal/queue continuation will continue: ${result.message}`,
      );
    else if (result.status === "skipped" && result.severity === "warning")
      notifyWarning(
        ctx,
        `Post-completion action ${running.type} skipped: ${result.message ?? "no reason provided"}`,
      );
  }
  return current;
}

export function createNoopPostCompletionActionRunner(
  disabledReason: string,
): PostCompletionActionRunner {
  return {
    async run(input) {
      return { ok: true, actionId: input.action.id, status: "skipped", message: disabledReason };
    },
  };
}

function completeActionState(
  action: PostCompletionActionState,
  result: PostCompletionActionRunResult,
): PostCompletionActionState {
  const now = Date.now();
  if (result.ok && result.status === "done")
    return { ...action, status: "done", completedAt: now, updatedAt: now };
  if (result.ok)
    return {
      ...action,
      status: "skipped",
...
function persistActionState(
  ...
  const actions = (goal.postCompletionActions ?? []).map((candidate) =>
...
function actionsChanged(
  previous: PostCompletionActionState[] | undefined,
  next: PostCompletionActionState[],
): boolean {
