...
import { queueHandoffReason } from "../domain/budget.js";
import { sendQueueHandoff } from "../queue/steering.js";
import { cloneQueuedGoal, getQueueRevision, type QueuedGoal } from "../persistence/queue-store.js";
import type { GoalQueueSteeringReason, GoalState } from "../domain/types.js";
...
export function decideTerminalContinuationTicket(
  goal: GoalState | null,
  queue: QueuedGoal[],
  opts: ContinuationTicketOptions = {},
): ContinuationTicket {
...
export function revalidateContinuationTicket(
  ...
  if (getQueueRevision() !== ticket.queueRevision)
    return { ok: false, reason: "queue_revision_changed" };
  if (queueHandoffReason(goal) !== ticket.reason)
    return { ok: false, reason: "terminal_reason_changed" };
  return { ok: true };
}

export function refreshContinuationTicketQueueRevision(
  ticket: ContinuationTicket,
): ContinuationTicket {
...
export function dispatchContinuationTicket(pi: ExtensionAPI, ticket: ContinuationTicket): boolean {
  ...
  return sendQueueHandoff(pi, ticket.reason, {
    goalId: ticket.goalId,
    triggerTurn: ticket.triggerTurn,
    deliverAs: ticket.deliverAs,
    force: ticket.force,
  });
}
