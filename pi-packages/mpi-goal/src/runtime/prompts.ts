import {
  BUDGET_LIMIT_PROMPT_ID,
  CONTINUATION_PROMPT_ID,
  PAUSE_PROMPT_ID,
} from "../domain/constants.js";
import { evaluateCompletionFloor } from "../domain/floor.js";
import { buildFloorContinuationGuidance, selectFloorWorkCard } from "../domain/floor-steering.js";
import { escapeXml } from "../domain/format.js";
import type { GoalState, GoalSteeringDetails, GoalTelemetrySnapshot } from "../domain/types.js";

export function buildContinuationPrompt(
  goal: GoalState,
  telemetry: GoalTelemetrySnapshot | null = null,
): { content: string; details: GoalSteeringDetails } {
...
Do not call update_goal unless the goal is complete. Do not mark a goal complete merely because the budget is nearly exhausted or because you are stopping work.`,
    details: {
      goalId: goal.goalId,
      kind: "continuation",
      promptId: CONTINUATION_PROMPT_ID,
      createdAt: Date.now(),
    },
  };
}

export function buildPausePrompt(goal: GoalState): {
  content: string;
  details: GoalSteeringDetails;
} {
  return {
    content: `The user has paused the active mpi-goal.
...
export function buildBudgetLimitPrompt(goal: GoalState): {
  content: string;
  details: GoalSteeringDetails;
} {
...
Do not call update_goal unless the goal is actually complete.`,
    details: {
      goalId: goal.goalId,
      kind: "budgetLimit",
      promptId: BUDGET_LIMIT_PROMPT_ID,
      createdAt: Date.now(),
      reason: "budget",
    },
  };
...
function floorContinuationSection(
  goal: GoalState,
...
function budgetLines(goal: GoalState, includeRemaining: boolean): string {
  ...
  const timeBudget =
    goal.timeBudgetSeconds === undefined ? "none" : `${goal.timeBudgetSeconds} seconds`;
  const tokenFloor =
    goal.minTokensBeforeWrapUp === undefined ? "none" : `${goal.minTokensBeforeWrapUp} tokens`;
  const timeFloor =
    goal.minTimeSecondsBeforeWrapUp === undefined
      ? "none"
      : `${goal.minTimeSecondsBeforeWrapUp} seconds`;
  const lines = [
  ...
  if (includeRemaining) {
    const tokensRemaining =
      goal.tokenBudget === undefined ? "unknown" : Math.max(0, goal.tokenBudget - goal.tokensUsed);
    const timeRemaining =
      goal.timeBudgetSeconds === undefined
        ? "unknown"
        : Math.max(0, goal.timeBudgetSeconds - goal.timeUsedSeconds);
    lines.push(
      `- Time remaining: ${timeRemaining === "unknown" ? "unknown" : `${timeRemaining} seconds`}`,
    );
