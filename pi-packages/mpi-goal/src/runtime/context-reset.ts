    ...
    async run(input) {
      ...
      if (input.action.mode === "clear") {
        ...
        if (flag !== "1" && flag !== "true" && flag !== "yes" && flag !== "on") {
          return {
            ...
            status: "skipped",
            severity: "warning",
            message: "clear mode disabled by PI_GOAL_CONTEXT_RESET_CLEAR",
          };
        }
      }
      const ctx = commandContext;
      if (!ctx?.navigateTree || !input.action.anchorEntryId)
        return {
          ok: false,
          actionId: input.action.id,
          status: "failed",
          severity: "warning",
          message:
            "Context summarization for tool-created goals requires /goal tools before completion.",
        };
      const result = await ctx.navigateTree(
        input.action.anchorEntryId,
        input.action.mode === "summarize"
          ? {
              summarize: true,
              customInstructions: `Summarize context for completed goal ${input.goal.goalId}.`,
      ...
      if (result.cancelled)
        return {
          ...
          status: "failed",
          severity: "warning",
          message: "Pi tree navigation was cancelled.",
        };
      return {
        ok: true,
        actionId: input.action.id,
        status: "done",
        message: `context reset ${input.action.mode} completed`,
      };
    },
  };
}
