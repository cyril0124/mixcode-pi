...
import { isContextResetClearEnabled } from "../domain/feature-flags.js";
...
export function createContextResetActionRunner(): PostCompletionActionRunner {
	return {
		async run(input) {
			...
			if (input.action.mode === "clear" && !isContextResetClearEnabled()) return { ok: true, actionId: input.action.id, status: "skipped", severity: "warning", message: "clear mode disabled by PI_GOAL_CONTEXT_RESET_CLEAR" };
