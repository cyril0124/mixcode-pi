...
export function registerGoalLifecycle(
	pi: ExtensionAPI,
	postCompletionRunner: PostCompletionActionRunner = createNoopPostCompletionActionRunner("post-completion runner unavailable"),
...
function isGoalSteeringCustomType(customType: unknown): customType is string {
	return [CONTINUATION_MESSAGE_TYPE, BUDGET_LIMIT_MESSAGE_TYPE, PAUSE_MESSAGE_TYPE, QUEUE_MESSAGE_TYPE].includes(String(customType));
}
