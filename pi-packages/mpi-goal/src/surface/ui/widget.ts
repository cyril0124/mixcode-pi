...
export function goalWidgetFactory(
  goal: GoalState,
): (_tui: unknown, theme: GoalWidgetTheme) => GoalWidgetComponent {
...
function commandLine(status: GoalStatus, theme: GoalWidgetTheme): string {
  const commands = commandHint(status)
    .replace(/^Commands: /, "")
    .split(", ");
...
function floorLine(goal: GoalState, theme: GoalWidgetTheme): string {
  ...
  const state = floor.allFloorsMet
    ? theme.fg("success", "met")
    : theme.fg("warning", "before wrap-up");
