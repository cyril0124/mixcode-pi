...
import { goalWidgetFactory } from "./widget.js";
...
export async function promptContinueActiveGoal(ctx: ExtensionContext, _goal: GoalState): Promise<boolean> {
	const choice = await ctx.ui.select("Continue active goal?", ["Continue goal", "Leave idle"]);
	return choice === "Continue goal";
}
