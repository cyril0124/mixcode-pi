...
function registerGetGoalTool(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    label: "Get Goal",
    description:
      "Get the current mpi-goal state, including status, budget, token usage, and time usage.",
    promptSnippet: "Inspect the active mpi-goal objective and progress state.",
    promptGuidelines: [
      "Use get_goal to inspect an explicit active goal before deciding whether it is complete.",
    ],
...
function registerListGoalTemplatesTool(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    description:
      "List reusable .pi-goals templates for explicit natural-language goal creation requests.",
...
function registerCreateGoalTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
  pi.registerTool({
    name: "create_goal",
    label: "Create Goal",
    description: "Create a new mpi-goal only when the user explicitly asks for a persistent goal.",
    promptSnippet: "Create an explicit persistent mpi-goal when requested.",
    promptGuidelines: [
      "Use create_goal only when the user explicitly asks to create or pursue a persistent goal.",
      "Do not infer goals from ordinary tasks; normal user requests are not automatically mpi-goals.",
    ],
...
function registerCreateGoalFromTemplateTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
  pi.registerTool({
    ...
    description:
      "Resolve a reusable .pi-goals template from structured parameters and create the resulting persistent goal.",
...
function registerUpdateGoalTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
  pi.registerTool({
    ...
    label: "Update Goal",
    description:
      "Update the current mpi-goal when the user explicitly asks to edit, pause, resume, or complete it.",
    promptSnippet: "Update an explicit mpi-goal field or status when requested by the user.",
...
function registerClearGoalTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
  pi.registerTool({
    ...
    label: "Clear Goal",
    description:
      "Clear the current mpi-goal when the user explicitly asks to remove the persistent goal or hide the goal widget.",
    promptSnippet: "Clear the explicit persistent mpi-goal when requested by the user.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      return withSessionTool(ctx, async () => {
        ...
        const queueHint =
          queueSize > 0
            ? ` ${queueSize} queued goal${queueSize > 1 ? "s" : ""} available. Queue steering was sent to the agent context.`
            : "";
        return resultForGoal(
          null,
          getTelemetry(),
          goal ? `Goal cleared.${queueHint}` : `No goal was set.${queueHint}`,
        );
      });
    },
  });
...
type GoalUpdateResult =
  | { ok: true; goal: GoalState; prefix: string; budgetChanged?: boolean }
  | { ok: false; error: string };

function createGoalFromTool(
  pi: ExtensionAPI,
...
function createGoalFromTemplateTool(
  pi: ExtensionAPI,
  runtime: GoalToolRuntime,
  params: CreateGoalFromTemplateInput,
  ctx: ExtensionContext,
) {
  const invocationArgs = params.args?.trim() ? ` ${params.args.trim()}` : "";
  const intent = buildTemplateGoalIntent({
    invocation: `${params.template}${invocationArgs}`,
    postCompletionActions: params.post_completion_actions,
  });
  if (!intent.ok) return errorResult(intent.error);
  const created = createGoalWithPolicy(
    pi,
    runtime,
    {
      ...params,
      objective: intent.intent.objective,
      post_completion_actions: intent.intent.postCompletionActions,
    },
    ctx,
    { replaceCompleted: true, replaceBudgetLimitedForQueuedWork: true },
  );
  if (!created.details.error && intent.intent.kind === "template")
    created.details.resolved_template = {
      name: intent.intent.template,
      path: intent.intent.template,
    };
  return created;
}
...
function createGoalWithPolicy(
  ...
  if (current && !canReplaceCurrentGoal(current, runtime, policy))
    return errorResult(
      "A goal already exists. Use clear_goal or ask the user before replacing it.",
    );
  ...
  let goal = createGoalState({
    ...
    postCompletionActions: createPostCompletionActionStates(normalized.postCompletionActions),
  });
...
type NormalizedCreateGoalInput =
  | {
      ok: true;
      objective: string;
      postCompletionActions: Array<{ type: "context.reset"; mode: "clear" | "summarize" }>;
    }
  | { ok: false; error: string };

function normalizeCreateGoalInput(params: CreateGoalInput): NormalizedCreateGoalInput {
  const intent = buildDirectGoalIntent({
  ...
  if (!validation.ok)
    return {
      ok: false,
      error: validation.hint ? `${validation.message} ${validation.hint}` : validation.message,
    };
  ...
  const floorError = validateFloorConfig({
    tokenBudget: params.token_budget,
    timeBudgetSeconds: params.time_budget_seconds,
    minTokensBeforeWrapUp: params.min_tokens_before_wrap_up,
    minTimeSecondsBeforeWrapUp: params.min_time_seconds_before_wrap_up,
  ...
  return floorError
    ...
    : {
        ...
        postCompletionActions: intent.intent.postCompletionActions,
      };
}

function canReplaceCurrentGoal(
  current: GoalState,
  runtime: GoalToolRuntime,
  policy: { replaceCompleted: boolean; replaceBudgetLimitedForQueuedWork?: boolean },
): boolean {
  ...
  if (
    policy.replaceBudgetLimitedForQueuedWork &&
    current.status === "budgetLimited" &&
    (runtime.getQueueSize?.() ?? 0) > 0
  )
    return true;
  return false;
}
...
function goalCreatedPrefix(current: GoalState | null): string {
  ...
  if (current?.status === "budgetLimited")
    return "Goal created; replaced budget-limited goal for queued work.";
  return "Goal created.";
}

function updateGoalFromTool(
  pi: ExtensionAPI,
  runtime: GoalToolRuntime,
  ...
  if (update.goal.status === "active" && !canActivateGoal(update.goal)) {
    ...
    return errorResult(
      `Cannot resume: ${resource} budget is still exhausted. Raise the budget or use clear_goal before resuming.`,
    );
  }
  ...
  if (completionDecision.kind === "defer_and_steer")
    return floorCompletionDeferredResult(pi, ctx, goal, completionDecision);
...
function validateBudgets(tokenBudget?: number, timeBudgetSeconds?: number): string | undefined {
  if (tokenBudget !== undefined && (!Number.isInteger(tokenBudget) || tokenBudget <= 0))
    return "token_budget must be a positive integer when provided.";
  if (
    timeBudgetSeconds !== undefined &&
    (!Number.isInteger(timeBudgetSeconds) || timeBudgetSeconds <= 0)
  ) {
...
function buildGoalUpdate(goal: GoalState, params: UpdateGoalInput): GoalUpdateResult {
  ...
  const floorError = validateFloorConfig({
    ...
    timeBudgetSeconds: next.timeBudgetSeconds,
    minTokensBeforeWrapUp: next.minTokensBeforeWrapUp,
    minTimeSecondsBeforeWrapUp: next.minTimeSecondsBeforeWrapUp,
  });
  if (floorError) return { ok: false, error: floorError };
  if (floorResult.floorChanged && params.status === "complete")
    return {
      ok: false,
      error: "Floor edits and status complete must be separate update_goal calls.",
    };
  next =
    budgetResult.budgetChanged && params.status === undefined
      ? recomputeStatusAfterBudgetEdit(next)
      : next;
  if (params.status !== undefined) {
    const status = parseToolStatus(params.status);
    if (!status)
      return { ok: false, error: "status must be active, paused, or complete when provided." };
    next = { ...next, status };
  ...
  return {
    ok: true,
    goal: { ...next, updatedAt: Date.now() },
    prefix: next.status === "complete" ? "Goal achieved." : `Goal updated: ${changes.join(", ")}.`,
  };
}
...
function applyBudgetUpdates(
  ...
  if (params.token_budget !== undefined) {
    if (
      ...
      (!Number.isInteger(params.token_budget) || params.token_budget <= 0)
    )
      return { ok: false, error: "token_budget must be a positive integer or null when provided." };
  ...
  if (params.time_budget_seconds !== undefined) {
    if (
      ...
      (!Number.isInteger(params.time_budget_seconds) || params.time_budget_seconds <= 0)
    )
      return {
        ok: false,
        error: "time_budget_seconds must be a positive integer or null when provided.",
      };
    next = {
      ...next,
      timeBudgetSeconds:
        params.time_budget_seconds === null ? undefined : params.time_budget_seconds,
    };
...
function applyFloorUpdates(
  goal: GoalState,
  params: UpdateGoalInput,
  changes: string[],
): GoalUpdateResult & { floorChanged?: boolean } {
  let next = goal;
  let floorChanged = false;
  if (params.min_tokens_before_wrap_up !== undefined) {
    if (
      params.min_tokens_before_wrap_up !== null &&
      (!Number.isInteger(params.min_tokens_before_wrap_up) || params.min_tokens_before_wrap_up <= 0)
    )
      return {
        ok: false,
        error: "min_tokens_before_wrap_up must be a positive integer or null when provided.",
      };
    next = {
      ...next,
      minTokensBeforeWrapUp:
        params.min_tokens_before_wrap_up === null ? undefined : params.min_tokens_before_wrap_up,
    };
  ...
  if (params.min_time_seconds_before_wrap_up !== undefined) {
    if (
      params.min_time_seconds_before_wrap_up !== null &&
      (!Number.isInteger(params.min_time_seconds_before_wrap_up) ||
        params.min_time_seconds_before_wrap_up <= 0)
    )
      return {
        ok: false,
        error: "min_time_seconds_before_wrap_up must be a positive integer or null when provided.",
      };
    next = {
      ...next,
      minTimeSecondsBeforeWrapUp:
        params.min_time_seconds_before_wrap_up === null
          ? undefined
          : params.min_time_seconds_before_wrap_up,
    };
...
function queueHandoffAfterToolUpdate(
  runtime: GoalToolRuntime,
  ctx: ExtensionContext,
  previousGoal: GoalState,
  updatedGoal: GoalState,
): void {
  ...
  if (
    ...
    updatedGoal.status === "budgetLimited" &&
    isBudgetExhausted(updatedGoal)
  ) {
    if ((runtime.getQueueSize?.() ?? 0) > 0)
      runtime.sendQueueHandoff?.("goal-budget-limited", {
...
function recomputeStatusAfterBudgetEdit(goal: GoalState): GoalState {
  ...
  return isBudgetExhausted(goal)
    ? { ...goal, status: "budgetLimited" }
    : { ...goal, status: "active" };
}

function telemetryForUpdate(
  goal: GoalState,
  completionDecision: CompletionDecision,
): GoalTelemetrySnapshot | null {
  if (goal.status === "budgetLimited" && isBudgetExhausted(goal))
    return noteBudgetLimit(getTelemetry());
  if (completionDecision.kind !== "allow_with_reason") return getTelemetry();
  const telemetry = getTelemetry();
  return telemetry
    ? {
        ...telemetry,
        noMoreValuableWorkReason:
          completionDecision.reason === "max_budget_requires_wrap_up"
            ? "max_budget_requires_wrap_up"
