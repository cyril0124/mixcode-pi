...
function registerGetGoalTool(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "get_goal",
		label: "Get Goal",
		description: "Get the current mpi-goal state, including status, budget, token usage, and time usage.",
...
function registerUpdateGoalTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
	pi.registerTool({
		name: "update_goal",
		label: "Update Goal",
		description: "Update the current mpi-goal when the user explicitly asks to edit, pause, resume, or complete it.",
...
function registerClearGoalTool(pi: ExtensionAPI, runtime: GoalToolRuntime): void {
	pi.registerTool({
		name: "clear_goal",
		label: "Clear Goal",
		description: "Clear the current mpi-goal when the user explicitly asks to remove the persistent goal or hide the goal widget.",
