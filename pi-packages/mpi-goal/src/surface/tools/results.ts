...
export function resultForGoal(
  ...
  const details: ToolDetails = {
    ...
    remaining_time_seconds: remainingTime(goal),
    floor: goal ? evaluateCompletionFloor(goal) : undefined,
  };
  if (
    goal?.status === "complete" &&
    (goal.tokenBudget !== undefined || goal.timeBudgetSeconds !== undefined)
  ) {
  ...
  return {
    content: [
      { type: "text" as const, text: `${prefix ? `${prefix}\n` : ""}${formatToolGoal(goal)}` },
    ],
    details,
  };
}
...
export function resultForTemplates(templates: ReturnType<typeof listGoalTemplateMetadata>) {
  const lines = templates.map((template) => {
    ...
    const placeholders = template.requiredPlaceholders.length
      ? ` placeholders=${template.requiredPlaceholders.join(",")}`
      : " placeholders=none";
  ...
  return {
    content: [
      {
        type: "text" as const,
        text: lines.length ? lines.join("\n") : "No reusable goal templates found.",
      },
    ],
    details: { goal: getGoal(), telemetry: getTelemetry(), templates } as ToolDetails,
  };
}
...
export function errorResult(error: string) {
  return {
    ...
    details: { goal: getGoal(), telemetry: getTelemetry(), error } as ToolDetails,
  };
}
...
export function formatToolGoal(goal: GoalState | null): string {
  ...
  const timeBudget =
    goal.timeBudgetSeconds === undefined
      ? "none"
      : `${formatElapsed(goal.timeBudgetSeconds)} (${remainingTime(goal)} seconds remaining)`;
  const floor = evaluateCompletionFloor(goal);
  const tokenFloor =
    floor.minTokensBeforeWrapUp === undefined
      ? undefined
      : `Token wrap-up floor: ${goal.tokensUsed} / ${floor.minTokensBeforeWrapUp}${floor.tokenFloorMet ? " (met)" : ` (${floor.tokensRemainingBeforeWrapUp} remaining)`}`;
  const timeFloor =
    floor.minTimeSecondsBeforeWrapUp === undefined
      ? undefined
      : `Time wrap-up floor: ${formatElapsed(goal.timeUsedSeconds)} / ${formatElapsed(floor.minTimeSecondsBeforeWrapUp)}${floor.timeFloorMet ? " (met)" : ` (${floor.timeSecondsRemainingBeforeWrapUp} seconds remaining)`}`;
  return [
    `Goal: ${goal.status}`,
