...
test("parseQueueBlockItems: keeps continuation lines and trims outer blanks", () => {
	const items = parseQueueBlockItems(["1. first objective", "   detail line", "", "2. second objective"].join("\n"));
...
test("parseGoalTemplateInvocation: separates flags, boolean flags and -- args", () => {
	assert.deepEqual(parseGoalTemplateInvocation('review --scope "src/core" --strict -- check the parser'), {
		name: "review",
...
test("parseGoalTemplateInvocation: supports --key=value and rejects empty input", () => {
	assert.deepEqual(parseGoalTemplateInvocation("review --scope=src"), { name: "review", flags: { scope: "src" }, args: "" });
