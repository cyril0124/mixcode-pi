 ...
 *
 * State is namespaced by session id (see state/store.ts), so the ctx is also
 * the source of the session id to write under.
 */
export function reconstructTodoState(
	ctx: Parameters<typeof replayFromBranch>[0] & Parameters<typeof sessionIdFromCtx>[0],
): void {
	replaceState(sessionIdFromCtx(ctx), replayFromBranch(ctx));
}
...
export function registerTodoTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			// State is per session: resolve the id from the execution context so
			// concurrent tabs mutate their own todo list, not a shared one.
		...
		renderCall(args, theme, _context) {
			// renderCall's context carries no session id; the subject lookup it
			// would do is purely cosmetic (accent label vs `#id`). Pass an empty
			// state so it falls back to `#id` rendering rather than risk reading
			// another session's tasks.
			return renderTodoCall(args as never, theme, EMPTY_STATE);
		},
...
export function registerTodosCommand(pi: ExtensionAPI): void {
	pi.registerCommand(COMMAND_NAME, {
		...
		handler: async (_args, ctx) => {
			...
			const state = getState(sessionIdFromCtx(ctx));
