/**
 * Vendored from the upstream open-source package `@juicesharp/rpiv-config`
 * (version 1.20.0, MIT, author: juicesharp,
 * https://github.com/juicesharp/rpiv-mono, directory packages/rpiv-config).
 *
 * Inlined here because the Pi extension loader's bare-import alias table does
 * not include the `@juicesharp/*` scope, so `import ... from
 * "@juicesharp/rpiv-config"` cannot resolve once this extension lives under
 * `pi-packages/`. Content is unchanged from upstream except for this header.
 *
