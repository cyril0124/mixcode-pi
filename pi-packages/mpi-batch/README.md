...
Pi discovers the skill through `pi.skills`. MixCode installs it under `<agentDir>/extensions/mpi-batch/`, where `index.ts` contributes the `skills/` directory through `resources_discover`.
