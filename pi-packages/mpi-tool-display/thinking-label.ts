...
function handleThinkingContextEvent(
  event: unknown,
  ctx: ExtensionContext | undefined,
  ...
  try {
    ...
    if (sanitizedMessages !== event.messages && Array.isArray(sanitizedMessages)) {
      // Keep shared references in sync; the return value below is the formal
      // contract (runner reassigns from handlerResult.messages).
      event.messages.splice(0, event.messages.length, ...(sanitizedMessages as unknown[]));
    }
    return event.messages as AgentMessage[];
  } catch (error) {
...
export function registerThinkingLabeling(pi: ExtensionAPI): void {
  ...
  pi.on("context", async (event, ctx) => {
    const messages = handleThinkingContextEvent(event, ctx);
