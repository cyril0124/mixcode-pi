...
function resolveLanguageFromPath(rawPath: string | undefined): string | undefined {
	...
	try {
		...
		// getLanguageFromPath throws on unmapped extensions; treating that as
		// "no language" degrades to the plain stripped line, which is safe.
		void error;
		return undefined;
	}
}
...
function createCodeLineHighlighter(language: string | undefined): CodeLineHighlighter {
	if (!language) {
		return (line) => stripAllEscapes(line);
	}
	...
	return (line) => {
		...
		const sanitized = highlightSanitizedLine(line, language);
		cache.set(line, sanitized);
		return sanitized;
	};
}

function highlightSanitizedLine(line: string, language: string): string {
	// File content is untrusted: strip every escape before highlighting so only
	// highlighter-generated SGR reaches the theme pipeline.
	const stripped = stripAllEscapes(line);
	let highlighted: string;
	try {
		highlighted = highlightCode(stripped, language)[0] ?? stripped;
	} catch (error) {
		// highlightCode can throw on tokenizer edge cases; falling back to the
		// stripped line is display-only degradation.
		void error;
		highlighted = stripped;
	}
	return sanitizeAnsiForThemedOutput(highlighted);
}
...
function formatMetaEntryRows(entry: DiffMetaEntry, width: number, theme: DiffTheme, wordWrap: boolean): RenderedRow[] {
	const normalized = stripAllEscapes(normalizeCodeWhitespace(entry.raw));
...
function collectConsecutiveLineEntries(
	...
	while (index < entries.length) {
		...
		if (candidate?.kind !== "line" || candidate.lineKind !== lineKind) {
			break;
		}
...
function tokenizeInlineDiff(input: string): Array<{ value: string; start: number; end: number }> {
	...
	// Local change: loop restructured for biome noAssignInExpressions.
	for (;;) {
...
function extractThemeBackgroundAnsi(text: string): string | undefined {
	...
	// Local change: loop restructured for biome noAssignInExpressions.
	for (;;) {
		const match = ANSI_SGR_PATTERN.exec(text);
		if (match === null) {
			break;
		}
...
function readThemeAnsi(theme: DiffTheme, kind: "fg" | "bg", slot: string): string | undefined {
	try {
		return lookupThemeAnsi(theme, kind, slot);
	} catch (error) {
		// Theme accessors may throw on unknown slot names; dropping the color
		// is display-only degradation.
		void error;
		return undefined;
	}
}

function lookupThemeAnsi(theme: DiffTheme, kind: "fg" | "bg", slot: string): string | undefined {
...
function colorizeSegment(
	...
	try {
		...
		// theme.fg may throw on unknown color slots; plain text is display-only
		// degradation.
		void error;
		themedText = text;
	}
