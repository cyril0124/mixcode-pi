...
| `"<tool>": {}` | 空模式对象合法，但不贡献任何规则，行为与省略该键完全一致。解析阶段即丢弃该键，`/permission list`、overlay 和保存后的配置文件中都不会出现它。 |
...
{
  "bash": {
    "git push*": { "action": "deny", "message": "请让用户手动推送。" }
  },
  "external_directory": {
...
最终决策取工具规则与防护规则中更严格者（`deny` > `ask` > `allow`）。没有 `external_directory` 规则时该防护关闭，键缺失或写空对象都一样；写 `"*": "ask"` 可把关所有检测到的外部路径。模式末尾 `/` 会被忽略，因此 `"../"` 匹配父目录本身，`"../*"` 匹配父目录中的内容。
