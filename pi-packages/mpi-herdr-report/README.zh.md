...
每次发送尝试前，扩展查询 `pane.process_info`，要求 `foreground_processes`
包含自身 PID。状态上报、会话上报和通知均需通过此检查。查询失败、不受支持或
返回无效应答时，扩展跳过发送，下一次周期刷新再检查。查询超时为 500 ms。
并发发送共享尚未完成的查询；后续发送需重新查询。
...
退出钩子不能等待 socket I/O，因此使用最近一次所有权检查的结果。检查成功时
记录窗格 ID 与 socket 路径；检查失败或 PID 不匹配时清除记录。若仍有已核实的
窗格，钩子启动独立的 `herdr pane release-agent` 子进程，其序号高于该进程
尚未完成的上报。`HERDR_BIN_PATH` 指定可执行文件，默认使用 `PATH` 中的
`herdr`。清理要求 CLI 可用且 Herdr 服务可达。
