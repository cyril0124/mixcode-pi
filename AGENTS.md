...
- Batch API changes must synchronize executors (`src/core/batch-lua.ts`: `.lua`; `src/core/batch-ts.ts`: `.ts`/`.mts`/`.js`/`.mjs`), root stubs (`mixcode.lua`: snake_case; `mixcode-batch.d.ts`: camelCase globals), and `docs/batch-scripts.md` + `.zh.md`. `test/batch-ts.test.ts` checks TS stub/runtime compatibility at compile time; check Lua stubs and docs manually. `examples/**/*.ts` is included in `tsconfig.json` and must pass `bun run typecheck`.
