...
- Read-only scrolling surfaces (pagers, log and content viewers, diff views) in `src/` and `pi-packages/` alike accept vim keys alongside the arrow keys: `j`/`k` one line, `Ctrl+D`/`Ctrl+U` half page, `g`/`G` top/bottom, `q` close. Reference implementations: the chat pager (`handleVimModeKey`, `src/ui/app-key-handlers.ts`) and `pi-packages/mpi-diff-viewer/diff-viewer.ts`. Surfaces with a query input (command palette, `/models` and `/workdir` pickers, extension manager, workspace overlay) are excluded: every printable character is search text there, and where `Ctrl+U` is bound on them it clears the query instead of scrolling. The hint line lists the bindings the surface actually handles; when width is tight, drop hints from the middle first and keep the scroll and close hints.
