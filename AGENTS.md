...
- Commit-time gate: `.pre-commit-config.yaml` runs `bun run lint` (whole repo, `pass_filenames: false`), mirroring the CI lint step. It is opt-in per clone — enable with `prek install` (or `pre-commit install`); without it commits are ungated. The hook stashes unstaged changes for the duration of the run, so a failing lint leaves the working tree untouched.
