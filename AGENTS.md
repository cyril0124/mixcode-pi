...
- Read-only scrolling surfaces (pagers, log and content viewers, diff views) in `src/` and `pi-packages/` alike accept vim keys alongside the arrow keys: `j`/`k` one line, `Ctrl+D`/`Ctrl+U` half page, `g`/`G` top/bottom, `q` close. Reference implementations: the chat pager (`handleVimModeKey`, `src/ui/app-key-handlers.ts`) and `pi-packages/mpi-diff-viewer/diff-viewer.ts`. Surfaces with a query input (command palette, `/models` and `/workdir` pickers, extension manager, workspace overlay) are excluded: every printable character is search text there, and where `Ctrl+U` is bound on them it clears the query instead of scrolling. The hint line lists the bindings the surface actually handles; when width is tight, drop hints from the middle first and keep the scroll and close hints. Such a surface may own a destructive action when it is armed behind an inline confirmation that replaces the hint line, is confirmed only by `y`, and is cancelled by every other key, `q` and `Esc` included.
...
- Commit-time gate: `.pre-commit-config.yaml` runs `bun run format` then `bun run lint` (whole repo, `pass_filenames: false`). Lint mirrors the CI gate; format has no CI counterpart and is enforced only here. It is opt-in per clone. Enable it with `prek install` (or `pre-commit install`); without it commits are ungated. The hook stashes unstaged changes for the duration of the run, so a failing gate leaves the working tree untouched. `bun run format` rewrites files in place and still exits 0, so the commit is failed by pre-commit's "files were modified by this hook" check — re-stage the rewritten files and commit again.
