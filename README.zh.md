...
- ⚡ **双队列精准调度**：区分轮次中紧急干预（Steer 队列，在下一个工具执行完毕时即时纠偏）与轮次后有序排队（`/follow-up` 队列），交互流畅不打断。
...
| 快捷键 | 作用域 | 动作 | 说明 |
|---|---|---|---|
| `Tab` / `Shift+Tab` | 全局 | 下一个 / 上一个 Tab | 轮转 Tab。补全打开或 Zen 模式下不切换（用 `Ctrl+T`）。 |
...
| `Ctrl+U` | 输入框/队列 | 出队 / 进入 Vim | 弹出排队消息回输入框；空队列预备进入 Vim。 |
