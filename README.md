...
- **`mpi-transcript`**: View transcript slices — effective LLM context, chatlog, thinking, latest replies — in `$EDITOR` (`/transcript`).
...
- **`mpi-bash`**: Default bash timeout plus a foreground window that detaches long commands to the background and reports their exit code when they finish; `/bash-jobs` opens any background command's full log.
